...
def test_imports():
    ...
    try:
        ...
        print("  OK: Explorer and Farmer RLModules imported")
...
def test_spaces():
    ...
    try:
        ...
        for role in ["gatherer", "explorer", "farmer"]:
            ...
            print(f"  OK: {role}: obs {len(obs_space.spaces)} keys, action {len(act_space.spaces)} keys")

        return True
    except Exception as e:
        print(f"  FAIL: Space building failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_config():
    ...
    try:
        ...
        for role in ["gatherer", "explorer", "farmer"]:
            ...
            print(f"  OK: {role} -> {cls.__name__}")

        return True
    except Exception as e:
        print(f"  FAIL: Config factory failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_reward():
    ...
    try:
        ...
        print(f"  OK: gatherer reward = {r_g:.4f}")
        ...
        print(f"  OK: explorer reward = {r_e:.4f}")
        ...
        print(f"  OK: farmer reward = {r_f:.4f}")

        # Test potentials
        phi_e = explorer_potential(obs_dummy)
        phi_f = farmer_potential(obs_dummy)
        print(f"  OK: explorer_potential = {phi_e:.4f}, farmer_potential = {phi_f:.4f}")

        return True
    except Exception as e:
        print(f"  FAIL: Reward testing failed: {e}")
        import traceback
        traceback.print_exc()
        return False
