/* AI Utopia — mock data + role helpers.
 *
 * ============================ BACKEND-WIRING SEAM ============================
 * This module is the SINGLE source of all data the GUI renders. Today every
 * export below is hard-coded mock data ported from the prototype's `data.js`.
 *
 * A future step will replace these constants with calls to a real backend
 * (FastAPI over the Py4J bridge / identity.db / training logs) WITHOUT touching
 * any component. Components import named exports from here; swap the
 * implementation, keep the shape. See gui/README.md "BACKEND WIRING — TODO".
 *
 *   agents       -> identity.db (ULIDs, roles, skins) + live bridge state
 *   logs         -> tailed training/agent log stream
 *   epochs       -> training progress.csv / Ray result.json
 *   activity     -> per-role action counts over time
 *   rewardCurve  -> reward history from training run
 * ===========================================================================
 */
...
export const getRoleColor = (r) => roleColors[r] || '#7A7A99';
...
export const epochs = Array.from({ length: 24 }, (_, i) => ({
  epoch: 1247 - i,
...
export const rewardCurve = Array.from({ length: 120 }, (_, i) => ({
  epoch: 1128 + i,
