...
/**
 * M2 PLOW: till soil (Grass/Dirt → Farmland, age 0).
 *
 * Params: target_location (3-tuple or serialized BlockPos).
 * Success: block changed to FARMLAND (age 0).
 *
 * Sequencing per tick:
 *   1. Parse target location from action JSON.
 *   2. If target not in range (>4.5 blocks), walk toward it.
 *   3. If target is Grass/Dirt and in reach, till it (1 tick, deterministic).
 *   4. If tilled successfully, COMPLETED; if not tillable, IMMEDIATE_FAILURE.
 */
public class PlowSkill implements SkillExecutor {
    ...
    public SkillResult start(ServerPlayerEntity agent, JsonObject action, MinecraftServer server) {
        ...
        if (!action.has("target_location")) {
            failureReason = "PLOW requires target_location array";
        ...
        this.ticksRemaining = action.has("timeout_ticks")
            ...
            : 600L;  // shorter timeout for short-distance skill
    ...
    public SkillResult tick(ServerPlayerEntity agent, MinecraftServer server) {
        ...
        if (distSq > REACH_RADIUS_SQ + 1e-3) {
            ...
            if (lastPos != null && here.squaredDistanceTo(lastPos) < STALL_DIST_EPSILON_SQ) {
                ...
                if (stuckTicks >= STALL_TICK_BUDGET) {
                    failureReason = "plow stalled — cannot reach target";
                    return SkillResult.FAILED_TIMEOUT;
                }
            } else {
                stuckTicks = 0;
        ...
        // Within reach — check if block is tillable (Grass/Dirt)
        BlockState state = world.getBlockState(targetPos);
        String blockId = Registries.BLOCK.getId(state.getBlock()).toString();
        if (!blockId.contains("grass") && !blockId.contains("dirt") || blockId.contains("podzol")) {
            failureReason = "target block is not grass or dirt: " + blockId;
            return SkillResult.IMMEDIATE_FAILURE;
        }

        // Till the block (set to Farmland, age 0)
        BlockState farmed = Blocks.FARMLAND.getDefaultState();
        world.setBlockState(targetPos, farmed, Block.NOTIFY_ALL);
        world.syncWorldEvent(2001, targetPos, Block.getRawIdFromState(state));
        world.emitGameEvent(agent, GameEvent.BLOCK_CHANGE, targetPos);
