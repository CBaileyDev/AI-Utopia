"""Explorer RoleRLModule — 8-way bearing producer for forest discovery.

§D1a — Explorer uses position + richness score + nearest resources to infer
a compass bearing (Discrete 8) that guides the Lumberjack's navigation.
The policy reuses the same CoreEncoder + SharedBackbone + CTDE critic as
Gatherer, with a minimal role-specific encoder (flatten richness + nearest
resources) and an actor head that outputs 8 logits for bearing selection.
"""
...
class ExplorerRoleRLModule(TorchRLModule, ValueFunctionAPI):
    """Per-role policy module for Explorer (forest discovery via bearing inference).
    ...
    def setup(self) -> None:
        ...
        self.role = cfg["role"]
        assert self.role == "explorer", (
            f"ExplorerRoleRLModule requires role='explorer', got {self.role!r}"
        )
        ...
        # Set the action distribution class. Explorer outputs Discrete(8) for bearing only.

        if self.role == "explorer":
            child_struct, input_lens = explorer_action_dist_config()
    ...
    def _forward(self, batch: dict, *, with_value: bool) -> dict:
        ...
        role_feat = self.role_encoder(obs_flat_t)  # (B*T, 64)
        pixel_feat = torch.zeros(
            core_feat.size(0), self._pixel_zero_dim, device=core_feat.device
        )  # (B*T, 64)
        fused = torch.cat([core_feat, role_feat, pixel_feat], dim=-1)  # (B*T, 384)

        # Reshape to (B, T, 384) for LSTM
        ...
        action_dist_inputs_flat = self.actor_head(hidden_flat, goal_flat)  # (B*T, 8)
        ...
        if with_value:
            # CTDE critic: M2 pilot, dummy agents at indices 1, 2, 3.
            n = core_input.size(0)  # B*T
            all_agents = torch.zeros(n, 4, core_input.size(-1), device=core_feat.device)
            all_agents[:, 1, :] = core_input  # explorer is agent index 1
